...
class The7_Inc_Shortcodes_VCParams {
	...
	public function register_params() {
		...
		$this->compatible_vc_add_shortcode_param( 'dt_radio_image', array( $this, 'dt_image_param' ), $script_url );
	...
	public function dt_icon_settings_field($settings, $value)
		{
			...
			if($param_name == "project_link_icon"){
				$icons = array('icon-portfolio-p211', 'icon-portfolio-p212', 'icon-portfolio-p208', 'icon-portfolio-p204','icon-portfolio-p201', 'icon-portfolio-p206', 'icon-portfolio-p203','icon-portfolio-p205');
			}
...
if ( class_exists( 'WPBakeryVisualComposerAbstract' ) ) {
	...
	function presscore_vc_register_custom_vc_pie_script() {
		wp_register_script( 'vc_dt_pie', PRESSCORE_THEME_URI . '/inc/shortcodes/vc_extend/jquery.vc_chart.js', array('jquery', 'waypoints', 'progressCircle'), THE7_VERSION );
	}
...
foreach ( array( 'dt_portfolio_masonry' ) as $shortcode_tag ) {
	// Posts.
	add_filter( "vc_autocomplete_{$shortcode_tag}_posts_callback", array( $posts_autocomplete, 'suggester' ) );
	add_filter( "vc_autocomplete_{$shortcode_tag}_posts_render", array( $posts_autocomplete, 'renderer' ) );
